...
const AI_MODELS_DATA = [
  {
    ...
    subtitle: "Realistic video avatar interview board with real-time speech-to-text voice recognition",
    ...
    summary: "Simulate high-stakes cadre selection and viva voce interviews. The AI generates cadre-specific technical questions, evaluates spoken voice responses in real time, and produces an official MoSPI PDF scorecard.",
    workflow: [
      {
        step: 1,
    ...
    metrics: { latency: "< 1.2s", accuracy: "97.4%", framework: "SankhyaIQ Oral Engine" },
  },
  {
    id: "assignments",
    title: "AI Generated Case Study Assignments",
    shortName: "Case Study Rubrics",
    subtitle: "Real-world MoSPI operational scenarios with instant rubric-based AI grading",
    icon: FaFileAlt,
    ...
    summary: "Solve real-world official statistical scenarios from NSSO, CSO, and state DES. Submit your analysis for instant in-depth AI grading across 4 official rubric dimensions.",
    workflow: [
      {
        step: 1,
    ...
    metrics: { latency: "< 2.0s", accuracy: "98.1%", framework: "SankhyaRubric Evaluator" },
  },
  {
    id: "copilot",
    title: "SankhyaCopilot AI Domain Assistant",
    shortName: "Statistical Copilot",
    subtitle: "24/7 official statistical methodology tutor and circular guidance copilot",
    icon: BsRobot,
    ...
    summary: "A dedicated domain copilot for officers and students to understand complex statistical methodologies, national account sequences, sampling designs, and MoSPI circulars.",
    workflow: [
      {
        step: 1,
    ...
    metrics: { latency: "< 0.8s", accuracy: "99.0%", framework: "SankhyaCopilot Neural" },
  },
  {
    id: "mcq",
    title: "AI MCQ Question Authoring Studio",
    shortName: "MCQ Authoring",
    subtitle: "Automated text extraction from survey manuals and circulars into 4-option MCQs",
    icon: FaFileUpload,
    ...
    summary: "Upload official survey manuals, circulars, or training notes (PDF/TXT) to automatically generate structured 4-option MCQs with in-depth pedagogical explanations.",
    workflow: [
      {
        step: 1,
    ...
    metrics: { latency: "< 1.5s", accuracy: "96.8%", framework: "Doc2MCQ Parser" },
  },
  {
    id: "quizzes",
    title: "AI Diagnostic Quizzes & Test Engine",
    shortName: "Diagnostic Quizzes",
    subtitle: "On-demand timed diagnostic tests with question palettes and topic mastery diagnostics",
    icon: FaTasks,
    badge: "Adaptive Tests",
    glowColor: "shadow-emerald-500/20 ring-emerald-500/30 border-emerald-500/40",
    gradient: "from-emerald-500 via-teal-600 to-emerald-700",
    textColor: "text-emerald-600 dark:text-emerald-400",
    bgColor: "bg-emerald-50 dark:bg-emerald-950/60",
    route: "/quizzes",
    category: "Diagnostic Testing",
    summary: "Take timed, adaptive diagnostic quizzes on sampling, price statistics, national accounts, and governance. View real-time scorecards and topic-level mastery insights.",
    workflow: [
      {
        step: 1,
    ...
    metrics: { latency: "< 1.0s", accuracy: "98.5%", framework: "Adaptive Diagnostic" },
  },
  {
    id: "competencies",
    title: "AI Multi-Domain Competency Engine",
    shortName: "Competency Radar",
    subtitle: "Automated skill-gap assessment against ISS, SSS, and FOD Cadre standards",
    icon: FaBrain,
    ...
    summary: "Evaluates officer proficiencies across 4 domains (Statistical, Technical & Computational, Digital Governance, Behavioural/Managerial) and computes priority skill gaps.",
    workflow: [
      {
        step: 1,
    ...
    metrics: { latency: "< 1.1s", accuracy: "99.2%", framework: "CadreMatrix Analyzer" },
  },
  {
    id: "learning-path",
    title: "AI Learning Pathway Synthesizer",
    shortName: "iGOT Synthesizer",
    subtitle: "Personalized capacity roadmaps combining iGOT digital courses & NSSTA residential programmes",
    icon: FaCertificate,
    badge: "iGOT & NSSTA",
    ...
    summary: "Synthesizes an ordered, pedagogical learning pathway directly mapped to bridge your detected skill deficits using verified iGOT Karmayogi and NSSTA modules.",
    workflow: [
      {
        step: 1,
    ...
    metrics: { latency: "< 1.4s", accuracy: "97.8%", framework: "PathSynthesizer Engine" },
  },
];
...
const AiModelsHub = () => {
  ...
  const activeModel = AI_MODELS_DATA.find((m) => m.id === selectedModelId) || AI_MODELS_DATA[0];
      ...
      <PageTransition>
        ...
        >
          ...
          <div className="space-y-2 max-w-2xl relative z-10">
            <div className="flex flex-wrap items-center gap-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 text-xs font-bold uppercase tracking-wider border border-blue-400/20">
                <HiSparkles className="text-amber-400 animate-spin" size={13} />
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              Explore the full suite of specialized AI engines built for India's Official Statistical System. Inspect multi-phase workflows, input/output specifications, and launch any neural tool directly.
            </p>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/15 p-4 rounded-2xl text-center shrink-0 min-w-[150px] relative z-10 shadow-lg">
            <span className="text-[10px] font-extrabold text-blue-200 uppercase tracking-wider block">Neural Engines</span>
            <span className="text-2xl font-black text-emerald-400">{AI_MODELS_DATA.length} Models</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch lg:h-[780px]">
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md ${model.bgColor} ${model.textColor}`}>
                          {model.badge}
                        </span>
                    ...
                    <div className="shrink-0 pt-2 text-slate-400">
                      <FaArrowRight size={11} className={isSelected ? model.textColor : "opacity-40"} />
                    </div>
                  </motion.button>
                );
              })}
          ...
          {/* ======================================================== */}
          {/* RIGHT SIDE: SELECTED AI MODEL CANVAS (EQUAL HEIGHT)      */}
          {/* ======================================================== */}
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/70 text-center">
                    ...
                    <span className="text-sm sm:text-base font-black text-emerald-600 dark:text-emerald-400">{activeModel.metrics.latency}</span>
                  </div>
                  <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/70 text-center">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Benchmark Match</span>
                    <span className="text-sm sm:text-base font-black text-blue-600 dark:text-blue-400">{activeModel.metrics.accuracy}</span>
                  </div>
                  <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/70 text-center">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Neural Engine</span>
                    <span className="text-xs sm:text-sm font-black text-slate-800 dark:text-slate-200 truncate block mt-0.5">{activeModel.metrics.framework}</span>
                  </div>
                </div>
                    ...
                    {activeModel.workflow.map((w, idx) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className={`w-7 h-7 rounded-xl bg-gradient-to-tr ${activeModel.gradient} text-white text-xs font-black flex items-center justify-center shadow-xs`}>
                            {w.step}
                          </span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {activeModel.features.map((feat, idx) => (
                      ...
                      >
                        <FaCheckCircle className="text-emerald-500 shrink-0 mt-0.5" size={13} />
                        <span>{feat}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
                  <span className="text-xs text-slate-500 dark:text-slate-400">
                    Ready to run this model? Click launch to open the live interactive view.
                  </span>
